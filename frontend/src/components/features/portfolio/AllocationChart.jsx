...
const CustomPieTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-black text-white px-2 py-1 text-[9px] font-mono tracking-widest shadow-lg">
...
const AllocationChart = () => {
  return (
    <div className="border border-[#dcd9ce] p-6 bg-transparent h-full flex flex-col justify-between">
      {/* Top Half: Allocation */}
      <div className="mb-8">
        <p className="text-[9px] text-gray-400 uppercase tracking-[0.2em] font-bold mb-4">ALLOCATION BY CATEGORY</p>
        <div className="flex items-center gap-6">
          <div className="w-24 h-24 shrink-0 relative">
            ...
            {/* Center dot */}
            <div className="absolute inset-0 m-auto w-12 h-12 rounded-full bg-cream shadow-inner flex items-center justify-center"></div>
          </div>
          
          <div className="flex-1 space-y-2">
            {allocationData.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center text-[9px] tracking-[0.1em] font-bold">
      ...
      <div>
        <p className="text-[9px] text-gray-400 uppercase tracking-[0.2em] font-bold mb-4">YTD NET GAIN / LOSS</p>
        <div className="flex items-end justify-between h-12 px-2 border-b border-[#dcd9ce] pb-2">
          {gainLossData.map((data, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col items-center group relative w-full px-0.5">
                {/* Tooltip on hover */}
        ...
        <div className="flex justify-between px-3 mt-2 text-[7px] text-gray-400 font-bold uppercase">
          {gainLossData.map((d, i) => <span key={i}>{d.month}</span>)}
        </div>
      </div>
    </div>
  );
